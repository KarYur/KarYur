...
def price_quantity(dataframe):
    j, priceAll, quantity = 0, 0, 0
    for i in dataframe['model']:
        if i in d:
            priceAll += int(dataframe['price'][j])
            quantity += 1
        j += 1
    return priceAll, quantity

print((price_quantity(df_PC)[0] + price_quantity(df_Laptop)[0] + price_quantity(df_Printer)[0])/
      (price_quantity(df_PC)[1] + price_quantity(df_Laptop)[1] + price_quantity(df_Printer)[1]))